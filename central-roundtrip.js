(function(){
  ...
  function returnPayload(s,r){const base=s.service||s.title||'';return{reservationId:r?.id||null,reservationCode:r?.reservationCode||'',serviceId:s.id,reservationLeg:'return',client:r?.client||'',phone:r?.phone||'',phones:Array.isArray(r?.phones)?r.phones:[],people:r?.people||null,date:s.returnDate||'',returnDate:'',tour:'',service:s.returnService||invertRoute(base)||base,route:s.returnRoute||invertRoute(s.route||''),boarding:s.dropoff||'',dropoff:s.boarding||'',apartment:s.apartment||'',time:s.endTime||s.returnTime||s.startTime||s.time||'',amount:s.returnRepasseAmount??s.repasseAmount??'',responsible:s.responsible||r?.responsible||''}}
  function outboundPayload(s,r){return{reservationId:r?.id||null,reservationCode:r?.reservationCode||'',serviceId:s.id,reservationLeg:'outbound',client:r?.client||'',phone:r?.phone||'',phones:Array.isArray(r?.phones)?r.phones:[],people:r?.people||null,date:s.date||r?.date||'',returnDate:'',tour:s.tour||'',service:s.service||s.title||'',route:s.route||'',boarding:s.boarding||'',dropoff:s.dropoff||'',apartment:s.apartment||'',time:s.startTime||s.time||'',amount:s.repasseAmount??'',responsible:s.responsible||r?.responsible||''}}
  ...
  function renderReturns(){
    ...
    try{
      ...
      const roundTrips=services().filter(s=>s.date&&s.returnDate);
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('[data-central-prepare]');if(!b)return;const s=services().find(x=>String(x.id)===String(b.dataset.centralPrepare));if(!s?.date||!s.returnDate)return;e.preventDefault();e.stopImmediatePropagation();prepareOutbound(s.id)},true);
