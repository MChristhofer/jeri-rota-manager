(function(){
  ...
  if(!document.querySelector('link[href^="manager-services-section.css"]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='manager-services-section.css?v=20260825-1';document.head.appendChild(link);
  }
  ...
  if(!section){
    ...
    section.innerHTML=`
      <div class="section-heading"><div><p class="eyebrow">CADASTRO MESTRE</p><h2>Serviços e NET</h2><p>Cadastre aqui o que será selecionado nas reservas. Passeio, modalidade, rota, NET e parceiro ficam concentrados no próprio serviço.</p></div></div>
      <div class="manager-services-grid">
        <article class="panel manager-services-panel">
          <div class="panel-head"><div><p class="eyebrow">CATÁLOGO</p><h3>Serviço</h3></div><span id="managerServiceCount"></span></div>
          <form id="managerServiceForm" class="manager-service-form">
            <label class="full">Nome do serviço<input id="managerServiceName" placeholder="Ex.: Transfer Jijoca → Jeri" required></label>
            ...
            <label>Origem<input id="managerServiceOrigin" placeholder="Ex.: Jijoca"></label>
            <label>Destino<input id="managerServiceDestination" placeholder="Ex.: Jericoacoara"></label>
            <label>Sigla / identificação da rota<input id="managerServiceRoute" placeholder="Ex.: Jijoca-Jeri"></label>
            <label>Base do NET<select id="managerServiceBasis"><option value="per_person">Por pessoa</option><option value="per_vehicle">Por veículo</option><option value="fixed">Valor fixo</option></select></label>
            <label>NET unitário (R$)<input id="managerServiceNet" type="number" min="0" step="0.01" placeholder="0,00" required></label>
            <label>Venda padrão (R$)<input id="managerServiceSale" type="number" min="0" step="0.01" placeholder="Opcional"></label>
            <label>Regra financeira<select id="managerServiceReceipt"><option value="net_first">NET primeiro</option><option value="commission_first">Comissão primeiro</option></select></label>
            ...
            <label>Parceiro / motorista padrão<input id="managerServicePartner" placeholder="Ex.: Carlos / Agência X"></label>
            <label>WhatsApp padrão<input id="managerServicePartnerPhone" inputmode="tel" placeholder="Ex.: (88) 99999-9999"></label>
  ...
  function serviceFormRow(){return{
    name:byId('managerServiceName').value.trim(),category:byId('managerServiceCategory').value||null,modality:byId('managerServiceModality').value||null,
    ...
    pricing_basis:byId('managerServiceBasis').value,net_value:Number(byId('managerServiceNet').value)||0,
  ...
  function resetServiceForm(){editingServiceId=null;byId('managerServiceForm')?.reset();byId('managerServiceCategory').value='Transfer';byId('managerServiceModality').value='Compartilhado';byId('managerServiceBasis').value='per_person';byId('managerServiceReceipt').value='net_first';byId('managerServiceActive').value='true';byId('managerServiceSave').textContent='Salvar serviço';byId('managerServiceCancel').style.display='none'}
  function editService(item){editingServiceId=item.id;byId('managerServiceName').value=item.name||'';byId('managerServiceCategory').value=item.category||'Transfer';byId('managerServiceModality').value=item.modality||'Compartilhado';byId('managerServiceOrigin').value=item.origin||'';byId('managerServiceDestination').value=item.destination||'';byId('managerServiceRoute').value=item.route_code||'';byId('managerServiceBasis').value=item.pricing_basis||'fixed';byId('managerServiceNet').value=Number(item.net_value||0);byId('managerServiceSale').value=item.default_sale_value??'';byId('managerServiceReceipt').value=item.receipt_rule||'net_first';byId('managerServiceActive').value=String(item.active!==false);byId('managerServicePartner').value=item.default_partner_name||'';byId('managerServicePartnerPhone').value=item.default_partner_phone||'';byId('managerServiceSave').textContent='Salvar alterações';byId('managerServiceCancel').style.display='inline-flex';byId('managerServiceName').focus()}
  function renderServices(){
    ...
    host.innerHTML=services.map(x=>`<div class="manager-service-card"><div class="manager-service-main"><strong>${esc([x.name,x.modality].filter(Boolean).join(' · '))}</strong><small>${esc(x.category||'Serviço')} · ${esc(x.origin||'—')} → ${esc(x.destination||'—')}</small><div class="manager-service-tags"><span class="manager-service-tag">NET ${money(x.net_value)} ${x.pricing_basis==='per_person'?'por pessoa':x.pricing_basis==='per_vehicle'?'por veículo':'fixo'}</span><span class="manager-service-tag">${x.receipt_rule==='commission_first'?'comissão primeiro':'NET primeiro'}</span>${x.default_partner_name?`<span class="manager-service-tag">${esc(x.default_partner_name)}</span>`:''}<span class="manager-service-tag ${x.active?'':'inactive'}">${x.active?'Ativo':'Inativo'}</span></div></div><div class="manager-card-actions"><button class="mini-button" data-service-edit="${x.id}">Editar</button><button class="mini-button" data-service-toggle="${x.id}">${x.active?'Desativar':'Ativar'}</button></div></div>`).join('')
  }
