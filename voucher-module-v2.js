(function(){
  ...
  function reservationByRef(ref){
    const key=String(ref??'');
    return reservationsList().find(x=>[x.id,x.cloudId,x.reservationCode].some(value=>String(value??'')===key));
  }
  async function resolveReservation(ref){
    let r=reservationByRef(ref);
    if(r?.cloudId)return r;
    if(window.JeriCloudData?.refresh){
      try{await window.JeriCloudData.refresh()}catch(error){console.warn('Falha ao atualizar reservas antes do voucher:',error)}
      r=reservationByRef(ref);
    }
    return r||null;
  }
  ...
  function operationalMeta(value){if(!String(value||'').startsWith(OP_META_PREFIX))return{};try{return JSON.parse(decodeURIComponent(String(value).slice(OP_META_PREFIX.length)))}catch{return{}}}
  function serviceForVoucher(s,r){const meta=operationalMeta(s.responsible),catalogItem=(window.jeriServiceCatalog||[]).find(item=>String(item.id)===String(s.serviceCatalogId||'')),type=String(s.serviceType||meta.serviceType||catalogItem?.category||'').toLowerCase(),isTransfer=type==='transfer'||/transfer/i.test(catalogItem?.category||s.title||s.service||'');const imagePath=catalogItem?.voucher_image_path||'';const imageUrl=imagePath&&client?client.storage.from('service-images').getPublicUrl(imagePath).data.publicUrl:'';return{title:catalogItem?.name||s.title||s.service||s.tour||r.service||'Serviço',date:s.date||r.date,return_date:s.returnDate||null,time:s.startTime||s.time||meta.startTime||'',end_time:s.endTime||meta.endTime||'',modality:s.modality||meta.modality||catalogItem?.modality||'',vehicle:s.vehicle||meta.vehicle||catalogItem?.vehicle_type||'',route:isTransfer?(s.route||[s.origin||meta.origin||catalogItem?.origin,s.destination||meta.destination||catalogItem?.destination].filter(Boolean).join(' → ')):'',boarding:s.boarding||meta.boardingPoints?.[0]?.location||'',dropoff:s.dropoff||meta.dropoffPoints?.[0]?.location||'',notes:s.serviceNotes||meta.serviceNotes||'',image_path:imagePath,image_url:imageUrl}}

  function paymentLabel(r){const total=Number(r.amount)||0,paid=Number(r.paidAmount)||0;if(total>0&&paid>=total)return'Reserva quitada';if(paid>0)return'Sinal recebido';return'Pagamento pendente'}
  function snapshot(r,notes){
    const services=servicesList().filter(s=>String(s.reservationId)===String(r.id)).map(s=>serviceForVoucher(s,r));
    const total=Number(r.amount)||0,paid=Math.max(0,Number(r.paidAmount)||0),balance=Math.max(0,total-paid),finalServices=services.length?services:[serviceForVoucher({title:r.service,date:r.date,boarding:r.boarding||'',responsible:r.responsible||''},r)];
    return{reservation_code:r.reservationCode||String(r.id),client:r.client,phone:r.phone||'',email:r.email||'',period:{start:r.date,end:finalServices.map(s=>s.return_date||s.date).filter(Boolean).sort().at(-1)||r.date},people:Number(r.people)||1,passengers:String(r.client||'').split(/\s*[/,;]\s*/).filter(Boolean),services:finalServices,boarding:r.boarding||finalServices[0]?.boarding||'A definir',lodging:r.lodging||'',customer_notes:notes||'',payment_status:paymentLabel(r),amount:total,paid_amount:paid,balance};
  }
  ...
  async function previewReservation(id){
    const r=await resolveReservation(id);if(!r)return alert('Reserva não encontrada. Atualize a página e tente novamente.');if(!r.cloudId)return alert('A reserva ainda não foi sincronizada com o Supabase. Aguarde alguns segundos e tente novamente.');
    const current=vouchers.find(v=>String(v.reservation_id)===String(r.cloudId)&&v.status==='issued');const base=snapshot(r,current?.snapshot_data?.customer_notes||'');
  ...
  async function makePdf(v,upload=false){
    if(window.JeriVoucherPdf?.generate)return window.JeriVoucherPdf.generate(v,{upload});
    ...
    let y=64;const line=(label,value)=>{doc.setFont('helvetica','bold');doc.setFontSize(9);doc.setTextColor(...navy);doc.text(label.toUpperCase(),18,y);doc.setFont('helvetica','normal');doc.setTextColor(45,55,62);const lines=doc.splitTextToSize(String(value||'A definir'),165);doc.text(lines,18,y+6);y+=11+(lines.length-1)*5};line('Cliente',s.client);line('Período',`${fmtDate(s.period?.start)} a ${fmtDate(s.period?.end)}`);line('Passageiros',`${s.people||1} · ${(s.passengers||[]).join(', ')}`);line('Serviços',(s.services||[]).map(x=>`${fmtDate(x.date)} — ${x.title}${x.time?` — ${x.time}`:''}${x.route?` — ${x.route}`:''}`).join('\n'));line('Embarque',s.boarding);line('Pagamento',s.payment_status);if(v.show_value){const total=Number(s.amount)||0,paid=Number(s.paid_amount)||0,balance=Math.max(0,Number(s.balance??total-paid));line('Valor da reserva',money(total));line('Sinal / valor pago',money(paid));line(balance>0?'Saldo a pagar no embarque':'Situação',balance>0?money(balance):'Reserva quitada')}if(s.customer_notes)line('Observações',s.customer_notes);
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.voucherReservation)previewReservation(b.dataset.voucherReservation);const find=id=>vouchers.find(v=>v.id===id);if(b.dataset.voucherView)downloadStored(find(b.dataset.voucherView));if(b.dataset.voucherShare)share(find(b.dataset.voucherShare));if(b.dataset.voucherEmail)email(find(b.dataset.voucherEmail));if(b.dataset.voucherCancel)cancel(find(b.dataset.voucherCancel));if(b.dataset.voucherOpenReservation){const r=reservationByRef(b.dataset.voucherOpenReservation);if(r){setSection('reservas');openModal(r.id)}}if(b.dataset.voucherReissue)previewReservation(b.dataset.voucherReissue);if(b.dataset.voucherNew)previewReservation(b.dataset.voucherNew)});
