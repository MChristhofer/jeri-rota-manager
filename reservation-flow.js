(function(){
  ...
  function normalizeServiceDraft(service={},fallbackSale=null){
    ...
    const origin=serviceType==='transfer'?(service.origin||meta.origin||route[0]||''):'';
    const destination=serviceType==='transfer'?(service.destination||meta.destination||route[1]||service.dropoff||''):'';
    return{...service,legMode:service.legMode||meta.legMode||'',repasseAmount:window.JeriFinance.storedNet(service),serviceType,modality:service.modality||meta.modality||(/compartilh/i.test(service.service||'')?'Compartilhado':'Privativo'),origin,destination,route:serviceType==='transfer'?(service.route||[origin,destination].filter(Boolean).join(' → ')):'',startTime:service.startTime||service.time||meta.startTime||'',endTime:service.endTime||meta.endTime||'',serviceNotes:service.serviceNotes||meta.serviceNotes||'',hotel:service.hotel||meta.hotel||(serviceType==='hospedagem'?(service.service||service.title||''):''),locator:service.locator||meta.locator||'',vehicle:service.vehicle||meta.vehicle||(!String(service.responsible||'').startsWith(OP_META_PREFIX)?service.responsible||'':''),serviceName:service.serviceName||meta.serviceName||'',tour:service.tour||legacyTour,boardingPoints,dropoffPoints,legacyResponsible:meta.legacyResponsible||(!String(service.responsible||'').startsWith(OP_META_PREFIX)?service.responsible||'':''),saleTotal:service.saleTotal??fallbackSale??0};
  }
  ...
  function injectServiceEditor(){
    ...
    [byId('reservationTotalAmount'),byId('reservationReceivedAmount')].forEach(input=>{input?.addEventListener('focus',()=>input.select());input?.addEventListener('input',()=>syncPaymentSummary(false));input?.addEventListener('blur',()=>{input.value=formatMoneyInput(parseMoney(input.value));syncPaymentSummary(false)})});
  }
  ...
  function updateConditionalFields(card){
    ...
    const dateInput=card.querySelector('[data-field="date"]');const dateLabel=dateInput?.closest('label');if(dateLabel){const text=[...dateLabel.childNodes].find(node=>node.nodeType===Node.TEXT_NODE);if(text)text.textContent=type==='hospedagem'?'Check-in *':'Data de ida *'}
  ...
  function renderDrafts(){
    ...
    host.innerHTML=serviceDrafts.map((s,i)=>`<article class="reservation-service-draft operational-service-card" data-service-index="${i}" data-service-id="${escape(s.id)}" data-leg-mode="${escape(s.legMode||'')}"><div class="service-draft-top"><div><span class="service-number">${i+1}</span><strong data-service-title>Serviço ${i+1}</strong></div><div class="service-draft-actions"><button type="button" class="text-button duplicate-service-draft" data-index="${i}">Duplicar</button>${serviceDrafts.length>1?`<button type="button" class="text-button remove-service-draft" data-index="${i}">Excluir</button>`:''}</div></div><div class="service-draft-grid operational-service-grid">
      ...
      <label class="catalog-compat-field" data-types="passeio">Passeio contratado<input data-field="tour" value="${escape(s.tour||'')}"></label>
      <label data-types="hospedagem">Hotel / pousada existente<input data-field="hotel" value="${escape(s.hotel||'')}" readonly></label>
      <label data-types="transfer passeio">Horário de ida / início<input data-field="startTime" type="time" value="${escape(s.startTime||'')}"></label>
      <label data-types="transfer passeio">Horário de volta / fim <span class="optional-label">opcional</span><input data-field="endTime" type="time" value="${escape(s.endTime||'')}"></label>
      <label class="catalog-compat-field" data-types="transfer">Veículo<input data-field="vehicle" value="${escape(s.vehicle||'')}"></label>
  ...
  function persistDrafts(reservation){
    ...
    const clean=serviceDrafts.map((draft,i)=>{
      const s={...draft};const display=serviceDisplay(s);const route=s.serviceType==='transfer'?[s.origin,s.destination].filter(Boolean).join(' → '):'';
