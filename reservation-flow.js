(function(){
  ...
  function normalizeServiceDraft(service={},fallbackSale=null){
    ...
    const normalizedType=serviceType==='outro'?'bate_volta':serviceType;
    return{...service,serviceType:normalizedType,modality:service.modality||meta.modality||(/compartilh/i.test(service.service||'')?'Compartilhado':'Privativo'),origin:service.origin||meta.origin||route[0]||'',destination:service.destination||meta.destination||route[1]||service.dropoff||'',startTime:service.startTime||service.time||meta.startTime||'',endTime:service.endTime||meta.endTime||'',serviceNotes:service.serviceNotes||meta.serviceNotes||'',hotel:service.hotel||meta.hotel||(normalizedType==='hospedagem'?(service.service||service.title||''):''),locator:service.locator||meta.locator||'',vehicle:service.vehicle||meta.vehicle||(!String(service.responsible||'').startsWith(OP_META_PREFIX)?service.responsible||'':''),serviceName:service.serviceName||meta.serviceName||((normalizedType==='bate_volta')?(service.service||service.title||''):''),legacyResponsible:meta.legacyResponsible||(!String(service.responsible||'').startsWith(OP_META_PREFIX)?service.responsible||'':''),saleTotal:service.saleTotal??fallbackSale??0};
  }
  ...
  const serviceTypeLabel=type=>({transfer:'Transfer',passeio:'Passeio',hospedagem:'Hospedagem',bate_volta:'Bate-volta'}[type]||'Serviço');
  ...
  function serviceDisplay(s){if(s.serviceType==='transfer')return['Transfer',s.origin&&s.destination?`${s.origin} → ${s.destination}`:'',s.modality].filter(Boolean).join(' · ');if(s.serviceType==='passeio')return s.tour||'Passeio';if(s.serviceType==='hospedagem')return s.hotel||'Hospedagem';if(s.serviceType==='bate_volta')return['Bate-volta',s.serviceName].filter(Boolean).join(' · ');return s.service||s.title||'Serviço'}
  function renderDrafts(){
    ...
    host.innerHTML=serviceDrafts.map((s,i)=>`<article class="reservation-service-draft operational-service-card" data-service-index="${i}"><div class="service-draft-top"><div><span class="service-number">${i+1}</span><strong data-service-title>${serviceTypeLabel(s.serviceType)} ${i+1}</strong></div>${serviceDrafts.length>1?`<button type="button" class="text-button remove-service-draft" data-index="${i}">Remover</button>`:''}</div><div class="service-draft-grid operational-service-grid">
      ...
      <label data-types="bate_volta">Destino / roteiro *<input data-field="serviceName" data-type-required="true" value="${escape(s.serviceName||'')}" placeholder="Ex.: Delta do Parnaíba"></label>
      <label data-types="transfer passeio bate_volta">Local de embarque *<input data-field="boarding" data-type-required="true" value="${escape(s.boarding||'')}" placeholder="Hotel, aeroporto ou ponto de encontro"></label>
      <label data-types="transfer passeio bate_volta">Horário de saída / início<input data-field="startTime" type="time" value="${escape(s.startTime||'')}"></label>
      <label data-types="transfer passeio bate_volta">Horário de retorno / fim<input data-field="endTime" type="time" value="${escape(s.endTime||'')}"></label>
