(function(){
  ...
  function serviceForVoucher(s,r){const meta=operationalMeta(s.responsible),type=String(s.serviceType||meta.serviceType||'').toLowerCase(),isTransfer=type==='transfer'||/transfer/i.test(s.title||s.service||'');return{title:s.title||s.service||s.tour||r.service||'Serviço',date:s.date||r.date,return_date:s.returnDate||null,time:s.startTime||s.time||meta.startTime||'',end_time:s.endTime||meta.endTime||'',modality:s.modality||meta.modality||'',vehicle:s.vehicle||meta.vehicle||'',route:isTransfer?(s.route||[s.origin||meta.origin,s.destination||meta.destination].filter(Boolean).join(' → ')):'',boarding:s.boarding||meta.boardingPoints?.[0]?.location||'',dropoff:s.dropoff||meta.dropoffPoints?.[0]?.location||'',notes:s.serviceNotes||meta.serviceNotes||''}}
