(function(){
  ...
  function netFor(reservationId){return window.JeriFinance.total(servicesFor(reservationId).map(window.JeriFinance.storedNet))}
  ...
  function render(){
    ...
    const totals=list.reduce((acc,r)=>{const total=Number(r.amount)||0,paid=Math.min(Number(r.paidAmount)||0,total||Infinity),open=window.JeriFinance.balance(total,paid),net=netFor(r.id);acc.total+=total;acc.paid+=paid;acc.open+=open;acc.net+=net;return acc},{total:0,paid:0,open:0,net:0});
